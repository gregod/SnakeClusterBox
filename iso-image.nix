# Largely based on https://github.com/NixOS/nixpkgs/blob/master/nixos/modules/installer/cd-dvd/iso-image.nix
# Used under MIT License, Copyright (c) 2003-2023 Eelco Dolstra and the Nixpkgs/NixOS contributors
# Adapted to add specializations to iso boot menu
...
let
  ...
  specialisationCfg = concatStringsSep "\n" (map (name: ''
  
    LABEL boot-${name}
    MENU LABEL ${config.isoImage.prependToMenuLabel} ${config.system.nixos.label}${config.isoImage.appendToMenuLabel} ${name}
    LINUX /boot/${config.system.boot.loader.kernelFile}
    APPEND init=${config.system.build.toplevel}/specialisation/${name}/init ${toString config.boot.kernelParams}
    INITRD /boot/${config.system.boot.loader.initrdFile}
  '') (attrNames config.specialisation));


  specialisationCfgCopyRam = concatStringsSep "\n" (map (name: ''
  
    # A variant to boot with 'copytoram'
    LABEL boot-copytoram-${name}
    MENU LABEL ${config.isoImage.prependToMenuLabel} ${config.system.nixos.label}${config.isoImage.appendToMenuLabel} ${name} (copytoram)
    LINUX /boot/${config.system.boot.loader.kernelFile}
    APPEND init=${config.system.build.toplevel}/specialisation/${name}/init ${toString config.boot.kernelParams} copytoram
    INITRD /boot/${config.system.boot.loader.initrdFile}

  '') (attrNames config.specialisation));


    specialisationCfgGrub = concatStringsSep "\n" (map (name: ''

    menuentry '${config.isoImage.prependToMenuLabel} ${config.system.nixos.label}${config.isoImage.appendToMenuLabel} ${name}' {
    linux /boot/${config.system.boot.loader.kernelFile} \''${isoboot} init=${config.system.build.toplevel}/specialisation/${name}/init ${toString config.boot.kernelParams}
    initrd /boot/${config.system.boot.loader.initrdFile}
    }
  '') (attrNames config.specialisation));


    specialisationCfgCopyRamGrub = concatStringsSep "\n" (map (name: ''

    menuentry '${config.isoImage.prependToMenuLabel} ${config.system.nixos.label}${config.isoImage.appendToMenuLabel} ${name} (copytoram)' {
    linux /boot/${config.system.boot.loader.kernelFile} \''${isoboot} init=${config.system.build.toplevel}/specialisation/${name}/init ${toString config.boot.kernelParams} copytoram
    initrd /boot/${config.system.boot.loader.initrdFile}
    }
  '') (attrNames config.specialisation));
  ...
  baseIsolinuxCfg = ''
    ...
    ${specialisationCfgCopyRam}

    menu separator
    MENU BEGIN NO_RAM
    MENU TITLE Dont copy to ram options
    
    LABEL boot
    MENU LABEL ${config.isoImage.prependToMenuLabel} ${config.system.nixos.label}${config.isoImage.appendToMenuLabel}
    LINUX /boot/${config.system.boot.loader.kernelFile}
  ...
  efiDir = pkgs.runCommand "efi-directory" {
    ...
    #
     ...
     ${specialisationCfgCopyRamGrub}
    
    submenu 'No copy ram options' {
    ${specialisationCfgGrub}
    }
